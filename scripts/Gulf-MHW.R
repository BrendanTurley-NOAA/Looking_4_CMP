...
if(review_code == F){
  ...
  ### save intermediate file
  setwd("~/R_projects/ESR-indicator-scratch/data/intermediate_files")
  saveRDS(sst_rdt, 'sst_rdetrend_lm_brick.rds')
  sst_rdt3 <- readRDS('sst_rdetrend_lm_brick.rds')
  ...
  cellsize_km <- cellSize(sst_rdt3, unit='km') |> values() |> mean()
  ### save intermediate file
  setwd("~/R_projects/Looking_4_CMP/data")
  writeCDF(sst_rdt3, 'oisst_dtlm_gulfw.nc',overwrite=TRUE)
  dat <- nc_open('oisst_dtlm_gulfw.nc')
  data <- ncvar_get(dat, 'oisst_dtlm_gulfw')
  # writeCDF(sst_rdt3, 'oisst_dt_gulfw.nc',overwrite=TRUE)
  # dat <- nc_open('oisst_dt_gulfw.nc')
  # data <- ncvar_get(dat, 'oisst_dt_gulfw')
  lon <- ncvar_get(dat, 'longitude')
  lat <- ncvar_get(dat, 'latitude')
  ### lon/lat grid locations for spatial plots
  lon_lat <- expand.grid(lon = lon,lat = lat)
  ...
  ### this is the MHW detection function
  file_in <- 'oisst_dtlm_gulfw.nc'
  file_out <- 'mhw_dtlm_results.RData'
  # file_in <- 'oisst_dt_gulfw.nc'
  # file_out <- 'mhw_dt_results.RData'
  
  setwd("~/R_projects/Looking_4_CMP/data")
  mhw_dt_cube <- detect3(file_in = file_in,
                         return_type = "df", 
                         # clim_period = c("1982-01-01", "2011-12-31"))
...
mhw_cube <- mhw_dt_cube |>
  ...
  st_as_sf(coords = c("x", "y"), crs = 4326) ### make sf
### cutout shelf only
mhw_cube <- st_intersection(mhw_cube, kmk_shp)
# st_join(points_sf, polygons_sf, join = st_within)
...
png('mhw-surface-area-seasonal-plot.png',
...
mtext('Proportion of Gulf with Marine Heatwaves', side = 3, outer = TRUE, cex = 5/4, font = 2, line = 5/4)
dev.off()


setwd("~/R_projects/Looking_4_CMP/figs")
png('mhw-surface-dd-seasonal-plot.png'
    , width = 9, height = 6, units = 'in', res = 300)
par(mfrow = c(2,2), mar = c(3,5,2,1),
    oma = c(0,0,3,0))
...
mtext('Gulf Marine Heatwave Cummulative Intensity', side = 3, outer = TRUE, cex = 5/4, font = 2, line = 5/4)
dev.off()
...
event_no_yr <- aggregate(event_no ~ cell + year(index_start), data = mhw_dt_cube,
                        function(x) length(unique(x))) |>
...
mhw_slope <- matrix(gridcell_lm$slope*10, 52, 72)
### put into raster
...
setwd("~/R_projects/Looking_4_CMP/figs")
png('mhw-surface-spatial-plot.png',
...
event_no_yr <- aggregate(intensity_cumulative_abs ~ cell + year(index_start), data = mhw_dt_cube,
                         mean, na.rm = T) |>
  setNames(c('cell','year','event_no')) |> type.convert()
event_no_mean <- aggregate(event_no ~ cell, data = event_no_yr, mean, na.rm = T) |>
  merge(cell_ll, all = T) |>
...
setwd("~/R_projects/Looking_4_CMP/figs")
png('mhw-surface-spatial-plot2.png',
    width = 6, height = 10, units = 'in', res = 300)
par(mfrow=c(2,1))
...
### cutout shelf only
event_no_yr <- st_intersection(event_no_yr, kmk_shp)
# st_join(points_sf, polygons_sf, join = st_within)

aggregate(event_no ~ year, data = event_no_yr, mean, na.rm = T) |> plot(,typ='o')
